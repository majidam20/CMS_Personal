<%@ Page Title="" Language="C#" MasterPageFile="~/AdminCPMaster.master" AutoEventWireup="true" CodeFile="Products.aspx.cs" Inherits="ControlPanel_Products" %>
...
<table style="width: 100%;">
		<tr>
			<td colspan="2">
				<div class="HeaderTop" dir="rtl" style="padding:10px 10px 10px 0;">
					ابتدا&nbsp; 
                    دسته محصولات مربوطه را انتخاب و سپس در آن دسته، محصول جدیدی را درج و یا ویرایش نمایید.</div>
			</td>
		</tr>
		<tr>
			<td style="padding-top: 5px; width: 60%" align='right'>
                <asp:DropDownList Style="font-family: Tahoma;" ID="drpProductAlbum" runat="server"
                    AutoPostBack="True" OnSelectedIndexChanged="drpProductAlbum_SelectedIndexChanged">
                </asp:DropDownList>
            </td>
		...
		<tr>
			<td style="padding-top: 5px;" align='right' colspan="2" dir="rtl">
                <asp:Literal ID="ltrAlbumNull" runat="server" Visible="false"><div class="HeaderTop" style="color:Red;">دسته ای برای محصولات تعریف نشده است و یا دسته های موجود غیر فعال هستند.<br/> برای تعریف و ویرایش آلبومها به <a href="Albums.aspx" style="color:Green;">مدیریت آلبومها</a> مراجعه کنید.</div></asp:Literal>
            </td>
		</tr>
		<tr>
			<td colspan="2">
					...
					<tr>
						<td align="right">
							...
							<asp:RequiredFieldValidator ID="reqImage" runat="server" ControlToValidate="filupImage"
								ErrorMessage="&lt;strong&gt;تصویر محصول را وارد کنید&lt;/strong&gt;" Text="&lt;strong&gt;تصویر ویدئو را وارد کنید&lt;/strong&gt;"
								ValidationGroup="Fields"></asp:RequiredFieldValidator>
							<asp:FileUpload ID="filupImage" runat="server" />
						</td>
						<td class="Header" style="text-align: left; direction: rtl;">تصویر محصول:</td>
					</tr>
					<tr>
			...
			<asp:ListView ID="lstVideos" runat="server" GroupItemCount="3" 
				...
				<ItemTemplate>
					...
					<table>
						...
						<td colspan="2">
							...
							<asp:Image ID="Image1" runat="server" ImageUrl='<%# string.Format("~/ImageReSizer.ashx?aid={0}&if={1}&w={2}&h={3}&t={4}",Eval("AlbumID"), Eval("FileName"), 180, 180, "p") %>' />
							<br />
							<span class="title"><asp:Label ID="Label3" colspan="2" runat="server" Text='<%# Eval("Title")%>'></asp:Label></span>
						</td>
						</tr>
						<tr>
				...
				<EditItemTemplate>
					...
					<table>
						...
						<td colspan="2">
							<asp:Label ID="lblID" runat="server" Visible="false" Text='<%# Eval("ID") %>'></asp:Label>
							<asp:Label ID="lblImageFileNameLST" runat="server" Visible="false" Text='<%# Eval("FileName") %>'></asp:Label>
							<asp:Label ID="lblFATitle" runat="server" Visible="false" Text='<%# Eval("Title") %>'></asp:Label>
							<asp:Label ID="lblFAText" runat="server" Visible="false" Text='<%# Eval("Describ") %>'></asp:Label>
							<asp:Image ID="Image1" runat="server" ImageUrl='<%# string.Format("~/ImageReSizer.ashx?aid={0}&if={1}&w={2}&h={3}&t={4}",Eval("AlbumID"), Eval("FileName"), 180, 180, "p") %>' />
							<br />
							<span class="title"><asp:Label ID="Label3" colspan="2" runat="server" Text='<%# Eval("Title")%>'></asp:Label></span>
